...
class Simulation:
    ...
    def simulate(self, r, plot=False):
        ...
        self.altitude = np.linalg.norm(self.X[:i], axis=1) - theory.radius_earth
        ...
        axs[0, 1].plot(T[:i], (np.linalg.norm(self.X[:i], axis=1) - theory.radius_earth) / 1000)
        axs[0, 1].set_ylabel('altitude (km)')
    
        axs[0, 2].plot(T[:i], TR[:i], label="thrust")
        axs[0, 2].plot(T[:i], DR[:i], label="drag")
        axs[0, 2].set_ylabel('force magnitude (N)')
        axs[0, 2].legend()
    ...
    def print_post(self):
        print(f'max altitude: {np.max(self.altitude) / 1000:8.1f} km')
...
def rocket_0(s_vert_activate=50):
    r = Rocket()
...
def rocket_1():
    ...
    s0 = Stage(m_wet=61, m_dry=20, D_drag=0.150, engines=[e0],
            prop=cycle.propellants.PropEthanolLOX(),
            #prop=cycle.propellants.PropMethalox(),
            )
